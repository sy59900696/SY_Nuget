...
public static class Logger
{
    ...
    static Logger()
    {
        ...
        string _sLog4netConfig = "log4net.config";
        string _sAppConfig = "";
        try
        {
            string _sExe = System.IO.Path.GetFileNameWithoutExtension(Assembly.GetEntryAssembly().Location);
            _sAppConfig = _sExe + "_log4net.config";
        }
        catch (Exception)
        {
        }
        if (!string.IsNullOrWhiteSpace(_sAppConfig) && File.Exists(_sAppConfig))
        {
            log4net.Config.XmlConfigurator.ConfigureAndWatch(new FileInfo(_sAppConfig));
        }
        else if (File.Exists(_sLog4netConfig))
        {
            log4net.Config.XmlConfigurator.ConfigureAndWatch(new FileInfo(_sLog4netConfig));
        }
        else
        {
            M_FileAppender();
